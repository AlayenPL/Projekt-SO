...
int SysVMessageQueue::remove() {
    ...
    if (msgctl(msqid_, IPC_RMID, nullptr) < 0) { perror("msgctl(IPC_RMID)"); return -1; }
    msqid_ = -1;
    return 0;
}
...
int SysVMessageQueue::send_req(int tourist_id, int tourist_pid) {
    ...
    if (msgsnd(msqid_, &msg, sizeof(BridgeReqMsg) - sizeof(long), 0) < 0) { perror("msgsnd(req)"); return -1; }
    return 0;
}
...
int SysVMessageQueue::recv_req(BridgeReqMsg* out) {
    ...
    BridgeReqMsg msg{};
    if (msgrcv(msqid_, &msg, sizeof(BridgeReqMsg) - sizeof(long), 1, 0) < 0) { perror("msgrcv(req)"); return -1; }
    if (out) *out = msg;
    return 0;
}
...
int SysVMessageQueue::send_done(int tourist_id, int tourist_pid) {
    ...
    if (msgsnd(msqid_, &msg, sizeof(BridgeResMsg) - sizeof(long), 0) < 0) { perror("msgsnd(done)"); return -1; }
    return 0;
}
...
int SysVMessageQueue::recv_done(int tourist_id, int tourist_pid) {
    ...
    BridgeResMsg msg{};
    if (msgrcv(msqid_, &msg, sizeof(BridgeResMsg) - sizeof(long), (long)tourist_pid, 0) < 0) { perror("msgrcv(done)"); return -1; }
    return 0;
}
